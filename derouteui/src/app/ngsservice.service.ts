...
const hackendpoint='http://detour-env-1.eba-weamzbgn.us-east-1.elasticbeanstalk.com/api/vi/detour';
const httpOptions = {
...
export class NgsserviceService {
  ...
  getpricingsolutions(origin:string,destination:string)
  {
    const format = 'yyyy-MM-dd';
    var ticketDate = new Date();
    const locale = 'en-US';
    ...
    const query:String="{"+
    "\"origin\":\"WAS\","+
    "\"prefDestinaionType\":\"Beach\","+
    "\"travelFrom\":\"5Dec\","+
    "\"travelTo\":\"10Dec\","+
   "\"travelPrefType\":\"Anything\","+
    "\"travelType\":\"International\","+
    "\"distance\":\"5hrs\"}";

    console.log(query);
    console.log(httpOptions);
    console.log(hackendpoint);
    return this.http.post<any>(hackendpoint ,query, httpOptions).pipe(
      tap((product) => console.log(`requested for pricing solutions data `),
      catchError(this.handleError<any>('request solutions shelves'))
    ));
  }
